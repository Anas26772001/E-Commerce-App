...
export default function ProductsInSpecificCategory() {
    const { productsInCategory, isLoading } = useContext(ProductsContext)
    ...
    return (
        <div className="container  my-20">
            {productsInCategory?.length == 0 ? <div className='  image-wrapper'>
                <img src='../../../public/no-product-found-DncxVh9z.png' className='object-contain mx-auto' alt='no-product-found-image' />
            </div> : <div className="grid  gap-5 grid-cols-1 md:grid-cols-2 lg:grid-0cols-3 xl:grid-cols-4">
                {productsInCategory?.map((product) => {
                    return <div key={product?._id} className='card relative animate__animated animate__backInUp group bg-white shadow-lg rounded-xl group'>
